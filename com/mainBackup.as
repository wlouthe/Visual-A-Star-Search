package com {
	...
	public class main extends MovieClip {
		...
		public function findPath(myX:Number, myY:Number)
		{
			...
			startPosX = 0;
			startPosY = 0;
			endPosX = stage.width-40;
			endPosY = stage.height-40;
			open = new Vector.<myPos>();
			closed = new Vector.<myPos>();
			pathFound = false;
			open.push(new myPos(startPosX,startPosY, 0, Math.abs(endPosX - startPosX) + Math.abs(endPosY - startPosY)));
			trace(open[0].x);
			while(!pathFound)
			{
				//trace("hello");
			...
			for(var i:Number = -1; i<=1; i++)
			{
				for(var j:Number =-1; j<=1;j++)
				{
					...
					if(myPoint.x+i >= 0 && myPoint.x+i <= stage.width && myPoint.y+j >= 0 && myPoint.y+j <= stage.height)
					{
						...
						if(openTest == false)
						{
							open.push(new myPos(myPoint.x+i,myPoint.y+j,myPoint.g+myG, Math.abs(endPosX - (myPoint.x+i)) + Math.abs(endPosY - (myPoint.y+j))));
						}
					}
				}
			}
