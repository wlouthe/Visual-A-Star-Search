package com {
	...
	public class main extends MovieClip {
		...
		public function main() {
			...
			sb.scaleX=.5;
			sb.scaleY=.5;
			cb.x = stage.width-cb.width-5;
			sb.x = stage.width-cb.width-sb.width-5;
			cb.y = cb.y + 5;
			sb.y = sb.y + 5;
			stage.addChild(sb);
			stage.addChild(cb);
			cb.addEventListener(MouseEvent.CLICK, clearClicked, false, 0, true);
			sb.addEventListener(MouseEvent.CLICK, switchClicked, false, 0, true);
			//addEventListener(MouseEvent.MOUSE_UP, mouseRelease,false,0,true);
		}
		public function clearClicked(m:MouseEvent)
		{
			clearBoard();
		}
		public function switchClicked(m:MouseEvent)
		{
			myPath = (myPath+1)%2;
		...
		public function clearBoard()
		{
			...
			stage.removeChild(sb);
			removeChild(drawGrid);
			stage.removeChild(path);
			drawGrid = new drawing(stage.width,stage.height);
			path = new Shape();
			stage.addChild(cb);
		...
		public function findPathAStar(myX:Number, myY:Number)
		{
			...
			//*/
		}public function findPathBestFirst(myX:Number, myY:Number)
		{
			startPosX = 0;
			startPosY = 0;
			endPosX = stage.width-1;
			...
			pathFound = false;
			pos[startPosX][startPosY] = new myPos(startPosX,startPosY, 0, (Math.sqrt(Math.pow(Math.abs(endPosX-startPosX),2)+Math.pow(Math.abs(endPosY-startPosY),2)))*10);
			...
			while(!pathFound)
			{
				...
				closed.push(open.shift());
				//trace("gumby");
				findAdjacentBestFirst(currentPos);
				//pathFound = true;
				modCount++;
			}
			trace("path has been found");
			...
			//if(currentPos.h == 0)
			{
				...
				while(currentPos.x != 0 || currentPos.y !=0)
				{
					...
					//trace("final x:"+(currentPos.x) +"; y:" + (currentPos.y) + "; g:" + (currentPos.g) + "; h:" + (currentPos.h) + "; f:" + (currentPos.f) + "; lastX:" + (currentPos.lastX) + "; lastY:" + (currentPos.lastY));
					path.graphics.lineTo(currentPos.x, currentPos.y);
				}
				//trace("x:"+(currentPos.x) +"; y:" + (currentPos.y) + "; g:" + (currentPos.g) + "; h:" + (currentPos.h) + "; f:" + (currentPos.f) + "; lastX:" + (currentPos.lastX) + "; lastY:" + (currentPos.lastY));
				stage.addChild(path);
			...
			for(var i:Number = -1; i<=1; i++)
			{
				for(var j:Number =-1; j<=1;j++)
				{
					...
					if(myPoint.x+i >= 0 && myPoint.x+i <= stage.width && myPoint.y+j >= 0 && myPoint.y+j <= stage.height)
					{
						...
						/*
						if(pos[myPoint.x+i][myPoint.y+j] != undefined)
						{
						...
						*/
						if((rgb & 0xffffff) != 0xffffff)
						{
							...
							//pos[myPoint.x+i][myPoint.y+j] = new myPos(myPoint.x+i,myPoint.y+j,10000,10000);
							//pos[myPoint.x+i][myPoint.y+j].myStatus = 2;
							continue;
						}
						//trace(pos[0]);
		...
		public function findAdjacentBestFirst(myPoint:myPos)
		{
			//trace(stage.width);
			//trace(stage.height);
			var myG:Number;
				...
				for(var j:Number =-1; j<=1;j++)
				{
					...
					if(myPoint.x+i >= 0 && myPoint.x+i <= stage.width && myPoint.y+j >= 0 && myPoint.y+j <= stage.height)
					{
						...
						if(pos[myPoint.x+i][myPoint.y+j] != undefined && pos[myPoint.x+i][myPoint.y+j].myStatus == 1)
						{
							...
							{
								pos[myPoint.x+i][myPoint.y+j].g = myPoint.g + myG;
								pos[myPoint.x+i][myPoint.y+j].f = pos[myPoint.x+i][myPoint.y+j].g;
								pos[myPoint.x+i][myPoint.y+j].lastX = myPoint.x;
								pos[myPoint.x+i][myPoint.y+j].lastY = myPoint.y;
								open.splice(open.indexOf(pos[myPoint.x+i][myPoint.y+j]),1);
								//*
								...
								else
								{
									for(var toSplice2:Number = 0; toSplice2 < open.length; toSplice2++)
									{
										if(open[toSplice2].g >= myPoint.g+myG)
										{
											numAdded2++;
											//trace(toSplice-1);
											//trace("adding pos x: "+myPoint.x+i);
						...
						if(openTest == false)
						{
							pos[myPoint.x+i][myPoint.y+j] = new myPos(myPoint.x+i,myPoint.y+j,myPoint.g+myG, (Math.sqrt(Math.pow(endPosX-(myPoint.x+i),2)+Math.pow(endPosY-(myPoint.y+j),2)))*10);
							pos[myPoint.x+i][myPoint.y+j].lastX = myPoint.x;
							pos[myPoint.x+i][myPoint.y+j].lastY = myPoint.y;
							pos[myPoint.x+i][myPoint.y+j].myStatus = 1;
							var numAdded:Number = 0;
							...
							{
								var myH:Number = (Math.sqrt(Math.pow(endPosX-(myPoint.x+i),2)+Math.pow(endPosY-(myPoint.y+j),2)))*10;
								//trace("posx:"+myPoint.x+" posy:"+myPoint.y+" h:"+	x2.h);
								for(var toSplice:Number = 0; toSplice < open.length; toSplice++)
								{
									//trace(open[toSplice].h);
									if(open[toSplice].h >= pos[myPoint.x+i][myPoint.y+j].h)
									{
										numAdded++;
										//trace(toSplice-1);
										//trace("adding pos x: "+myPoint.x+i);
			...
			//for each(var x2 in open)
			//{
			//	trace("posx:"+x2.x+" posy:"+x2.y+" h:"+	x2.h);
			//}
			//pathFound=true;
			if(open.length == 0) pathFound = true;
		}
